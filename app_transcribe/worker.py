...
# =============================================================================
# MeiliSearch Configuration (lazy singleton with retry)
# =============================================================================
...
def index_to_meilisearch(transcript_id: int, call_uid: str, text: str, language: str):
    ...
    Gracefully skips indexing if MeiliSearch is unavailable.
    """
    client = get_meili_client()
    if client is None:
        log.warning(f"MeiliSearch unavailable — skipping indexing for {call_uid}")
        return
